...
@section('title', 'Data Wali Murid - Admin - SIPEMA')
...
@push('scripts')
<script>
    // Data wali murid untuk detail modal (termasuk alamat)
    const waliData = [
        @foreach($wali as $w)
        {
            id: {{ $w->id }},
            ...
            siswa: [
                @if($w->siswa && $w->siswa->count() > 0)
                    ...
                    {
                        nama_lengkap: '{{ addslashes($siswa->nama_lengkap ?? "Nama tidak tersedia") }}',
                        nis: '{{ $siswa->nis ?? "-" }}',
                        kelas: '{{ addslashes($siswa->kelas->nama_kelas ?? "Tanpa Kelas") }}'
                    },
                    @endforeach
                @endif
            ]
