...
<div class="glass-card rounded-2xl p-4 mb-6">
    <form method="GET" action="{{ route('bendahara.tagihan.index') }}">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center space-x-2">
                            @if($item->status != 'lunas')
                                ...
                                <button 
                                    onclick="confirmDelete({{ $item->id }}, '{{ $item->siswa->nama_lengkap ?? 'Siswa' }}', '{{ \Carbon\Carbon::createFromDate($tahun, $bulan, 1)->translatedFormat('F') ?? '' }} {{ $tahun ?? '' }}')"
...
function confirmDelete(id, namaSiswa, periode) {
    ...
    // Set action form dengan route yang benar untuk bendahara
    deleteForm.action = `{{ url('bendahara/tagihan') }}/${id}`;
